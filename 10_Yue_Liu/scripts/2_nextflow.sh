...
if [ -z "$TMUX" ]; then
    SCRIPT="$(realpath "$0")"
    cd /home/gao/projects_2026H2/10_Yue_Liu/scripts
    tmux kill-session -t rnaseq_yueliu 2>/dev/null || true
    tmux new-session -d -s rnaseq_yueliu "bash '${SCRIPT}' 2>&1 | tee nextflow_run.log"
    echo "Nextflow 已在 tmux session 'rnaseq_yueliu' 启动"
    echo "查看: tmux attach -t rnaseq_yueliu"
    echo "日志: tail -f /home/gao/projects_2026H2/10_Yue_Liu/scripts/nextflow_run.log"
    exit 0
fi
