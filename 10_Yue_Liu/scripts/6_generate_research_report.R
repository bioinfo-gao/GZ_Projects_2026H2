...
report <- c(
  paste0("# Human Bulk RNA-seq Analysis Report — Yue Liu Project"),
  "",
  ...
  "| Group | Samples | Role |",
  "| :--- | :---: | :---: |",
  paste0("| A | A_1, A_2, A_3 | Control |"),
  paste0("| 16 | 16_1, 16_2, 16_3 | Treatment |"),
  "",
  paste0("Total samples: **", nrow(meta), "**  |  Comparison: 16 vs A"),
  "",
      ...
      lines <- c(lines,
        paste0("- **", comp_name, "**: ", s$total, " DEGs identified. ",
               if (s$up > s$down) paste0("Predominantly upregulated (", s$up, " up vs ", s$down, " down), ",
                 "suggesting activation of transcriptional programs in group 16 relative to control A.")
               else if (s$down > s$up) paste0("Predominantly downregulated (", s$down, " down vs ", s$up, " up), ",
                 "suggesting suppression of gene expression in group 16 relative to control A.")
  ...
  "| :--- | :---: |",
  paste0("| `DE_PCA_Results/DEG_*.csv` | Full DEG table (all genes, with log2FC, padj, raw counts) |"),
  paste0("| `DE_PCA_Results/PCA.pdf` | PCA plot |"),
  paste0("| `DE_PCA_Results/Volcano_*.png` | Volcano plot |"),
  paste0("| `DE_PCA_Results/Heatmap_top50_*.pdf` | Heatmap of top 50 DEGs |"),
