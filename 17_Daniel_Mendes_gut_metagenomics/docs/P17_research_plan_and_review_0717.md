...
- 2026-07-20 — **MAG pipeline 跑完（3328/3328 成功，0 fail）+ 下游整理 + Phase 2 交付**：
  1. nf-core/mag 于 10:18 完成，watchdog 确认后自动退出。GenomeBinning/QC 原始产物(BUSCO/CheckM2/DASTool，13G+)已按 `/git` skill 加入 `.gitignore`(只留 `bin_summary.tsv`)、commit（push 待用户放行）。
  2. **QC/GTDB-Tk 是跑在原始 per-binner bins(1060 行，跨 binner 有同一基因组的重复)上，不是跑在 DASTool 去冗余后的 184 个 final bins 上** —— 两者文件名不同(`XxxRefined-group-XX.NNN[_sub].fa` vs 原始 binner 命名)。`scripts/15_mag_final_bin_catalog.py` 按 binner+group+编号做名字映射（SemiBin2 原始用 `_NNN` 下划线、DASTool 改 `.NNN` 点，需专门转换），把 184 个 DASTool 最终 bin 逐一对回其 CheckM2/BUSCO/QUAST/GTDB-Tk 记录，100% 映射成功（0 unmapped）。这是本轮最关键的技术决策点，写入脚本注释供复核。
