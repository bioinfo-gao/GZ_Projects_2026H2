...
Three samples (mouse_28, mouse_29, mouse_46) show a "Bacterial 16S"-dominant rRNA profile (15.6–37.7%) rather than the eukaryotic-28S pattern seen in the other six.

The bacterial 16S database was run at a 90% sequence-identity threshold, a looser identity threshold is inherently more prone to registering matches on non-bacterial sequence. .

The 15.6–37.7% bacterial-16S figure for mouse_28/29/46 should be treated as **an upper bound, likely somewhat inflated**, rather than a precise measurement of bacterial contamination .
...
```
custom_research_report_20260627/
├── Mouse_rRNA_QC_0627.md          ← this report
├── qc/
│   ├── *_trimming_report.txt      ← Trim Galore reports (dimer/insert size data, Section 6.1)
│   └── multiqc_report.html        ← aggregated QC report (GC content, adapter content, all FastQC metrics)
├── rrna/
│   └── *.sortmerna.log            ← SortMeRNA per-sample logs (rRNA database breakdown, Section 6.2 & 7.2)
└── alignment/
    └── *.Log.final.out            ← STAR alignment summary logs (mapping rate, Section 6.3)
```
