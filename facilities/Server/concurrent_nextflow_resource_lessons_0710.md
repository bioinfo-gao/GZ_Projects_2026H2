...
**关键（如实更正）：不是"两个都降配"——实际只有 proj13 降配，proj14 全程跑常规满配。**
...
**为什么"只降配一个"也能守住 28 核预算——两处处理必须综合看：**
- proj13 启动时，proj14 **已跑完比对、进入变异调用尾段**（Mutect2/去重，CPU 轻）。实测 proj14 在重叠期
  平均只占 **~9.9 核**（对比它独跑比对期占满 **28.2 核**）——**它当时自己就用不满满配的 48 线程**，
  等于被所处阶段"自然降配"了。
- 因此真正需要人工压核的只有 **CPU 重的那一个（proj13 的比对期）**：把 proj13 降到 24 线程，
  和 proj14 尾段实际占用（~10 核）相加 ≈ 一直落在 28 核预算内。
- **教训**：并跑不必机械地"每个都降配"。**先判断每个作业当前所处阶段的真实资源占用**——已进入低 CPU 尾段的
  作业会自限，只需对仍在 CPU 重阶段的作业降配即可，避免不必要地拖慢那个满配也不会超预算的作业。

**⚠️ 该策略的有效前提与风险**：只在 proj14 停留在低 CPU 阶段时成立。**一旦 proj14 又进入 CPU 重阶段
（如 Study B 的比对），而 proj13 仍在跑，两个满/降配相加会冲破 28 核预算**——届时必须给 proj14 也临时挂一份
降配（或错开两者的比对期）。收尾侧：proj14 结束后，proj13 须换回自己的满配版 `local_resources.config`，
否则会一直背降配速度白白变慢。
（本段的核占用实测与提速评估详见 `13_Ellen_knockin_wgs/logs/key_operations_log.md` 的"并跑到底加快了没有"。）
...
5. **要守的是"两作业实际占用之和 ≤ 28 物理核"，不是机械地"每个都降配"。** 本次实测就只降配了 proj13
   （proj14 满配），因为 proj14 已在低 CPU 尾段、自己就用不满（~10 核）——只需给仍在 CPU 重阶段的那个降配。
   判断方法：先看各作业当前阶段的真实核占用（trace 的 realtime×%cpu），已自限的作业可留满配、不必无谓拖慢。
   **风险**：自限作业一旦回到 CPU 重阶段（如另一 study 的比对），相加就会超限，届时它也得临时降配或错开。
