...
if [ -z "$TMUX" ]; then
    SCRIPT="$(realpath "$0")"
    cd /home/gao/projects_2026H2/11_Qiuchen_Li/scripts
    tmux kill-session -t rnaseq_qiuchenli 2>/dev/null || true
    tmux new-session -d -s rnaseq_qiuchenli "bash '${SCRIPT}' 2>&1 | tee nextflow_run.log"
    echo "Nextflow 已在 tmux session 'rnaseq_qiuchenli' 启动"
    echo "查看: tmux attach -t rnaseq_qiuchenli"
    echo "日志: tail -f /home/gao/projects_2026H2/11_Qiuchen_Li/scripts/nextflow_run.log"
    exit 0
fi
