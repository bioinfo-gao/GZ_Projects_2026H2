...
- **All six samples carry the correct construct at the intended locus.** In every sample the expected human insert is recovered essentially in full (96.8–99.8% breadth) — at the practical ceiling for short-read whole-genome sequencing — together with read-pair bridging to the intended mouse target locus. No sample identity or labeling issue was found.
- **No credible off-target integration** was detected in any sample after filtering recurrent alignment artifacts.
- **Copy number is single-copy for five of six samples.** One MTTH sample (**MTTH_284**) carries the human *HTT* insert at **~3 copies at the on-target locus**; the uniform, full-length dosage increase with no second integration site is **most consistent with a head-to-tail concatemer (tandem array)**. We recommend confirming copy number and structure by ddPCR/Southern before using this founder.
...
In every sample the expected human insert is recovered essentially in full — to the limit of what short-read sequencing can resolve — with read-pair bridging to the intended mouse locus. No sample carries a construct other than its designated one.
...
| **MTTH_284** |  **1.60**  | **~3 copies at the on-target locus; uniform full-length dosage increase with no second site — most consistent with a head-to-tail concatemer. Confirm by ddPCR/Southern.** |

### 6.3 Sequence integrity

All human inserts are recovered at 96.8–99.8% breadth — effectively complete recovery, since the small shortfall from 100% falls in regions where the human insert is too similar to its mouse counterpart for short reads to be placed uniquely, and does not represent missing sequence — with no contiguous near-zero coverage block, indicating **no large-scale internal deletion or rearrangement**. Scattered low-coverage windows within the *HTT* inserts (e.g. MTTH_412) are non-zero (roughly 10–30% of local median, longest run ~3 kb) and reflect reduced mappability where human *HTT* is ~85% identical to mouse *Htt*, not sequence loss.
...
| Copy number           |    ~2 copies    |    single    |    single    | **~3 copies (concatemer)** |  single  |  single  |
...
**Overall:** All six samples are correctly targeted, structurally intact, single-copy humanizations free of detectable off-target integration — with the single exception that **MTTH_284 carries the human *HTT* insert at ~3 copies at the on-target locus, most consistent with a head-to-tail concatemer (tandem array)**, which we recommend confirming by ddPCR/Southern before using that founder.

## 8. Deliverable Files

```
custom_research_report_20260712/
├── Ellen_KnockIn_WGS_0712.md      ← this report
