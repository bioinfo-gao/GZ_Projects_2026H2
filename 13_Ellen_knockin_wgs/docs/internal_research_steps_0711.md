...
- **本文性质**: 内部工作文档(含调试细节、脚本 bug、方法局限),**不进客户交付**。客户版见
  `custom_research_report_20260712/Ellen_KnockIn_WGS_0712.md`。
...
**②补 — `<sample>.<TG>.offtarget_screen.tsv` 文件解读(供后续复用/答客户疑问):**
- **本质**:构建体读段落在预期靶点之外的**原始候选清单**,交付里保留供透明(已随附 `README_offtarget_artifact_filter.txt`),里面的"命中"是候选、非结论。
- **列含义**:`chrom/start/end`=候选脱靶位点(小鼠 GRCm39);`support_reads`=唯一比对(MAPQ≥20)指向该位点的构建体读段数(=证据强度);`site_depth`=该位点局部测序深度;`class`=`novel_offtarget?`(**带 `?` 即未确认候选,非判定结果**)。
- **真 vs 假判据**:真随机整合应**私有于单个样本**且支持读数较高;**跨无关品系反复出现的位点=比对假象**(读段错贴到重复/低复杂度区)。已核实:chr7:90091184(5样,三系都有)、chr6:128806035、chr10:121599139(各4样)——无关品系不可能共享同一真整合位点。
- **文件内两个佐证**:① `support_reads` 普遍很低(如 CD1A_B125 最高仅 14、多数 3–5);② 多个命中处 `site_depth`≈0(chr6:128806035=0、chr18:71556414=0.8、chr1:57262701=1.4),读段落在零深度处正是错贴特征。
- **用法**:只有"**仅单一样本出现 + 支持读数较高**"的位点才值得跟进;本项目 6 样过滤后无一满足 → 可信脱靶=0(与报告 §6.4 一致)。
...
- **要给出确切合子型所需的专项分析(此信息已从客户 0712 报告删除,仅留内部)**:需针对性做**结合部/断点跨越读段**分析——
  在人源插入与小鼠直系同源序列**足够分歧的子区**取唯一比对读段,或检测保留 WT 等位特有的 mouse-flank↔mouse-target
  结合部读段,判断原生 WT 等位是否仍存在。**技术上可行但不保证成功**(高保守子区仍被同源污染),**本轮未做**。
  因此客户报告不再挂 "would require targeted junction-read analysis and can be added if desired" 的服务承诺(不向客户
  dangle 未做/不保证的分析);若客户明确要求,再评估可行性后执行。

**④ breadth 96.8–99.8% = 短读长 WGS 的实际检出上限,≠ 插入不完整**(判读 + 客户沟通口径):
- 6 样人源插入 breadth 落在 **96.8–99.8%**(见 §5 表);不足 100% 的几个百分点,是人源插入与其替换的
  小鼠直系同源序列高度相似、短读段无法唯一比对的区域(典型如 MTTH 的 human *HTT* ~85% 同源 mouse *Htt*),
  属**比对度(mappability)上限,非序列缺失**。
- 整合完整性以"**有无连续近零覆盖块**"判定(本项目 6 样均无);散布的低覆盖窗深度**非零**(局部中位约 10–30%,
  最长连续 run ~3kb),正是上述同源区,**不是内部缺失/重排**。
- **客户沟通口径(重要)**:不能只写 "near-complete coverage (96.8–99.8% breadth)"——非生信读者会误读成
  "插入片段不完整"。应表述为 **"insert recovered essentially in full (96.8–99.8% breadth) — at the
  practical ceiling for short-read whole-genome sequencing"**:既给出数据,又用语言点明这是技术上限而非缺失。
  (本次 0712 报告的 §2/§6.1/§6.3 已按此口径改写。)
...
### 5.1 MTTH_284 多拷贝 / concatemer 详解(唯一偏离"干净单拷贝"的样本)

**拷贝数换算**:ratio = 人源插入唯一比对深度 ÷ 样本自身常染色体基线深度;基线 = 二倍体 **2 份**。
- ratio 0.5 → 1 份(杂合单拷贝,预期);ratio 1.0 → 2 份。
- **MTTH_284 ratio 1.60 → ≈ 3.2 份人源 HTT 插入**;同系 MTTH_524=0.45(~1份)、MTTH_412=0.71(~1–2份)均为单拷贝级。
  即 MTTH_284 比同系多带 2–3 倍拷贝(human_meandepth 32.3 vs 基线 20.2)。

**concatemer(串联多拷贝)含义**:打靶载体除预期"单份干净插入"外,常见非预期形式是**多份构建体首尾串联**
(head-to-tail tandem array)整合在同一位点——转基因/敲入的经典假象(供体浓度高、环状供体时尤甚),
表现为"看似单一位点但拷贝数成倍升高"。

**判为 concatemer(而非多位点分散整合)的证据**:
1. MTTH_284 确为 **on-target**(TG_MTTH breadth 99.8%、有桥接读段落回 Htt)——至少一份在设计靶点;
2. 插入**全长深度均匀升高**(whole-contig meandepth 22.7 vs 同系 9–13)——符合同位点多份串联,而非局部重复;
3. 脱靶经跨样复现假象过滤后**无第二个可信真位点**——多出的拷贝最可能是 Htt 位点上的串联,而非另有整合;
4. (附)该样 off-target 原始输出里出现较多 TG_MTTH↔TG_CD1A 跨构建体桥接,与"载体 DNA 量大(多拷贝)→结合部读段多"一致。
   严格区分"串联 vs 分散"需结合部/断点专项(本轮未做),但现有证据一致指向串联 concatemer。

**对客户的实际影响(为何"需注意")**:
- 多拷贝/concatemer humanization **不等同于干净单拷贝等位**:(a)表达量可能偏高或失调;
  (b)串联重复易重组/收缩,**遗传上不稳定**;(c)偏离"单拷贝人源化"设计初衷。
- **建议**:把 MTTH_284 作为 founder 传代前先确认——可经后代分离观察,或用 **ddPCR / Southern blot** 独立验证拷贝数与结构。
- 报告中已在拷贝数一栏单独标注 `MULTI_COPY/concatemer(follow-up)`,不与其余单拷贝样本混为一谈。
