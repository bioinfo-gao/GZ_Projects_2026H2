...
- **本文为权威版（canonical）**。英文对照版 [`analysis_plan_0715A.md`](analysis_plan_0715A.md) 冻结于
  2026-07-15 22:08，**按设计保持滞后、无需同步**，仅作历史快照留存
  （user directive 2026-07-16：一中一英保留，脱节没关系）。**任何修改只改本文。**
...
- 2026-07-16 — §4 补入 **annotation DB（gnomAD-AF + ClinVar）的下载位置/版本/来源 URL/复用方法**表
  （`/Work_bio/references/Homo_sapiens/GRCh38/annotation/`，3.4 GB 实测）；修复 **`MT`→`chrM` 改名陷阱**
  （原 `0_prep_annotation_dbs.sh` 产出 `chrMT`，会静默丢失全部 mito ClinVar 注释）：脚本已修，
  共享 DB 的 `clinvar_GRCh38.chr.vcf.gz` 已于 12:25 重建并验证（contig=`chrM`；总记录 4,439,617
  与原始一致；mito 3,125 条无丢失）。step 4 尚未运行，无既有结果受影响。完整复盘见 **§7「反面教训 2」**；
  §9 self-audit 加了 mito 注释非空校验；教训已写入 `/wgs` skill。
- 2026-07-16 — §4 annotation DB 表格改为列**完整绝对路径**（原先只给文件名+目录，需读者自行拼接），
  并标注 step 4 实际使用的是派生件 `clinvar_GRCh38.chr.vcf.gz`（非 NCBI 直接下载件）。
- 2026-07-16 — ⚠ **用户指出监控缺位**（Phase 2 从未启动、监控未在 tmux）：新增常驻看门狗
  `scripts/10_watchdog.sh`（tmux 会话 `pan_watch`，含 STALLED / LOW_CPU_UTILIZATION 新规则，已实测触发）；
  §6 运行顺序补入；完整复盘见 §7「反面教训 3」。
- 2026-07-16 — 新增串联教材 `事故复盘_提速反噬_resume失效_监控缺位_0716.md`（项目内）+ 跨项目通用版
  `facilities/Server/nextflow_pipeline/resume缓存失效与空转监控_教训_0716.md`；时间线数字全部由
  `.nextflow.log` 实测核实（ELC 6.36h/exit 129、cache 31 中 BWAMEM2 仅 1、监控空窗 11.0h）。
...
| # | step                                            |             tool             | script                                                           |
| :- | :---------------------------------------------- | :---------------------------: | :--------------------------------------------------------------- |
| 0 | 下载 gnomAD-AF + ClinVar DB → `/Work_bio/.../GRCh38/annotation/`（共享，见 §4） | wget + bcftools | `scripts/0_prep_annotation_dbs.sh`                             |
...
- **Annotation DB 的下载位置（共享、可复用，2026-07-15 实测）：**
  **`/Work_bio/references/Homo_sapiens/GRCh38/annotation/`**（合计 3.4 GB，`du -sh` 实测；与
  `GATK.GRCh38/`、`gatk_bundle/`、`human_gencode_v45/` 同级）。**故意不放项目目录内** —— 这两个 DB 与
  项目无关、体积不小，任何 human GRCh38 项目都能直接复用，放共享目录避免每个项目重下一份。
  下载由 `scripts/0_prep_annotation_dbs.sh` 完成（幂等：文件已存在则跳过，可安全重跑）。

  **完整绝对路径（复制即用）：**

  | 完整路径 | 大小 | 版本 | 来源 URL | 用途 |
  | :--- | :---: | :---: | :---: | :---: |
  | `/Work_bio/references/Homo_sapiens/GRCh38/annotation/af-only-gnomad.hg38.vcf.gz` (+`.tbi`) | 3.0 GB | GATK best-practices resource（v2-derived joint AF） | `https://storage.googleapis.com/gatk-best-practices/somatic-hg38/` | `INFO/AF` → rarity filtering；已 `chr`-prefixed 无需改名 |
  | `/Work_bio/references/Homo_sapiens/GRCh38/annotation/clinvar_GRCh38.vcf.gz` (+`.tbi`) | 184 MB | `fileDate=2026-07-06`（weekly 滚动） | `https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/clinvar.vcf.gz` | 原始下载件（Ensembl contig 命名 `1,2,…,MT`）；**不直接用于注释** |
  | **`/Work_bio/references/Homo_sapiens/GRCh38/annotation/clinvar_GRCh38.chr.vcf.gz`** (+`.tbi`) | 185 MB | 同上，派生（2026-07-16 12:25 重建） | 由上一行经 `bcftools annotate --rename-chrs chr_rename.txt` 生成 | **⭐ step 4 实际使用的就是这份**（contig 已改名为 `chr1,…,chrM`） |
  | `/Work_bio/references/Homo_sapiens/GRCh38/annotation/chr_rename.txt` | 202 B | — | 脚本生成 | contig 改名表（`MT chrM`，见下方陷阱） |

  > `clinvar_GRCh38.chr.vcf.gz` 是 step 4 唯一实际读取的 ClinVar 文件（见
  > `scripts/4_annotate_gnomad_clinvar.sh` 的 `CLINVAR=$DB/clinvar_GRCh38.chr.vcf.gz`）。
  > 它**不是**从 NCBI 直接下载的——是由同目录 `clinvar_GRCh38.vcf.gz` 本地改名派生而来，
  > 因此没有独立的来源 URL；删掉后重跑 `scripts/0_prep_annotation_dbs.sh` 即可重建。

  - **为什么需要 `.chr.` 那一份**：ClinVar 官方 VCF 用 Ensembl 命名（`1,2,…,X,Y,MT`），而我们的 calls 是
    GATK.GRCh38 命名（`chr1,…,chrX,chrY,chrM`）。`bcftools annotate` 要求 contig 名**逐字相同**，不符
    时**不报错、静默零注释**（同 §4 开头的坑）。gnomAD af-only 本身已是 `chr`-prefixed，**无需改名**。
  - **⚠ 线粒体命名陷阱**：改名表必须写 `MT chrM`，**不是 `chrMT`** —— GATK.GRCh38 的线粒体 contig 名是
    **`chrM`**。`for c in $(seq 1 22) X Y MT; do echo "$c chr$c"; done` 这种顺手的写法会产出 `chrMT`，
    导致全部 mitochondrial ClinVar 注释静默丢失。脚本已修正并加注释。
  - **ClinVar 是 weekly 滚动发布**：现存这份为 `fileDate=2026-07-06`。复用于**新**项目时若需最新
    pathogenicity，删掉 `clinvar_GRCh38*.vcf.gz*` 重跑 step 0 即可；**本项目已开跑的分析不要中途换版**
    （注释版本需与 report 记录一致）。gnomAD AF-only 是静态 resource，无需更新。
  - **以后复用（任何 human GRCh38 项目）**：直接指过去，无需重下 ——
    ```bash
    DB=/Work_bio/references/Homo_sapiens/GRCh38/annotation
    bcftools annotate -a $DB/af-only-gnomad.hg38.vcf.gz -c 'INFO/gnomAD_AF:=INFO/AF' ...
    bcftools annotate -a $DB/clinvar_GRCh38.chr.vcf.gz  -c 'INFO/CLNSIG,INFO/CLNDN' ...
    ```
    （与 `GATK.GRCh38/` 参考的复用方式对称，见上一条 §4 "下载位置 + 已持久化"。）
...
# ⚠ MANDATORY —— 起完长任务立刻挂常驻看门狗，不能只靠 Phase-1 三分钟启动确认（见 §7 反面教训 3）：
tmux new-session -d -s pan_watch "bash scripts/10_watchdog.sh"
#   ↑ 10 min 巡检直到结束；告警见 logs/watchdog_ALERTS.log；只告警不 kill
...
各 tmux 会话：`pan_wgs`(sarek) / **`pan_watch`(常驻看门狗)** / `pan_prep`(预装 env) / `pan_down`(下游编排器)。
**每个跑长任务的会话都必须有配对的常驻监控会话** —— 这是 2026-07-16 事故后的硬性要求。
...
> 📕 **本节三条「反面教训」的串联教材版**：[`事故复盘_提速反噬_resume失效_监控缺位_0716.md`](事故复盘_提速反噬_resume失效_监控缺位_0716.md)
> —— 把「提速反噬 → 监控缺位 → `-resume` 救不回」串成一条因果链并抽出可复用判据。
> 跨项目通用版见 [`facilities/Server/nextflow_pipeline/resume缓存失效与空转监控_教训_0716.md`](../facilities/Server/nextflow_pipeline/resume缓存失效与空转监控_教训_0716.md)。
...
### ⚠ 反面教训 2：ClinVar `MT`→`chrMT` 改名错误（2026-07-16 发现并修复）

与 §7 的 Spark 教训并列记录，供复盘。**这是本项目第二个"静默失败"类 bug——两次都不是程序报错，而是
程序愉快地跑完、给出看似正常的结果。**

- **Bug**：`scripts/0_prep_annotation_dbs.sh` 用
  `for c in $(seq 1 22) X Y MT; do echo "$c chr$c"; done > chr_rename.txt` 生成 contig 改名表。
  这个"顺手"的循环把 `MT` 拼成 **`chrMT`**，而 GATK.GRCh38 的线粒体 contig 实际叫 **`chrM`**
  （见 `Homo_sapiens_assembly38.fasta.fai`）。1–22/X/Y 全对，唯独 MT 错——**24/25 正确的错误最难发现**。
- **后果（若未发现）**：`bcftools annotate` 对 contig 名不符**不报错、不警告**，只是零命中 →
  全部 **3,125 条 mitochondrial ClinVar 注释静默丢失**。step 5 的 rare+functional 筛选会漏掉所有
  mito 致病变异（MT-ND/MT-TL/MT-RNR 等线粒体病相关基因），而 report 里看不出任何异常。
- **讽刺之处**：§4 开头**正是我自己写的**"contig-name mismatch 是最容易踩的坑，`bcftools annotate`
  不是报错、而是零注释静默通过"——写下这段警告的同一个脚本里就埋着这个 bug。
  **知道一个坑 ≠ 不会掉进去**；写下规则的动作本身会制造"我已经处理了"的错觉。
- **发现路径（可复用）**：不是靠读代码读出来的，而是用户问"DB 下载在哪"→ 为写文档去 `tabix -l`
  **实际列出 contig 名**时看见 `chrMT`。**教训：光看脚本源码看不出这个 bug（`chr$c` 语法完全正确），
  必须核对产物的实际内容与目标参考的实际 contig 名。**
- **修复 + 验证**（12:25）：改名表改为
  `{ for c in $(seq 1 22) X Y; do echo "$c chr$c"; done; echo "MT chrM"; }`，重建共享
  `clinvar_GRCh38.chr.vcf.gz` 并验证：contig=`chrM` ✓；总记录 4,439,617 与原始 VCF 一致（无丢失）✓；
  mito 3,125 条与原始 `MT` 数量相等 ✓。
- **代价 = 0（侥幸）**：step 4 尚未运行（sarek 仍在跑），无既有结果受影响、无需返工。
  **若晚发现几小时**，错误注释会流入 step 5 筛选与 client report，且因为"静默"而极可能一路交付出去。
- **举一反三（已写入 §9 self-audit + `/wgs` skill）**：
  1. **任何 `bcftools annotate` / 跨 DB 叠加，事后必须验一条目标 contig 上的记录拿到了非空注释**——
     零注释和"该位点确实没注释"在输出里长得一模一样。
  2. **改名/转换类操作要核对总数守恒**（`bcftools index -n` 新旧对比），别只看命令退出码 0。
  3. **边缘 contig（MT/chrM、chrY、alt/decoy）是命名分歧的高发区**，主染色体全对不代表没问题。

### ⚠ 反面教训 3：监控缺位 —— 这才是让教训 1 从 1h 变成 6.3h 的原因（2026-07-16 用户指出）

**这条是前两条的元教训**：Spark 那个坑本身不致命，**真正的损失来自 11 小时无人监控**。

- **事实**（`logs/phase1_monitor.log` 实证）：07-15 23:50 带 Spark 重启后，Phase-1 监控跑了 5 轮 ×30s、
  最后一行 `monitor done` 就**自行退出**（mtime 23:53）。**23:53 → 次日 10:52，11 小时零监控。**
  ELC 约 04:35 开始空转，按 CLAUDE.md 要求的 Phase-2（10–20 min 巡检）**最晚 05:00 就该发现**，
  实际拖到 10:52 —— **多烧了约 6 小时，且 ~53 核全程空转**。
- **违反的两条 MANDATORY 规则**：
  1. **「Phase 1 通过 ≠ 监控完成」**。CLAUDE.md 明写 Phase 1 之后要「switch to longer-interval
     background checks (every 10–20 min) **until completion**」—— Phase 2 从未启动。
  2. **「长任务监控必须在 tmux」**。Phase-1 监控是用 Bash `run_in_background` 跑的，与 agent 会话绑定，
     会话一结束即消失。`scripts/3_monitor.sh` 只是**一次性**查询脚本（tail 完就退），不是常驻循环。
- **更难看的是**：通用看门狗 `facilities/Server/nextflow_watchdog.sh` **早已存在**、`/corun` skill 也写明
  用法，本项目**压根没调用**。不是没工具，是没用。
- **但光"记得跑看门狗"也不够（影响修复方案，非辩解）**：通用看门狗只有 SESSION_END / LOW_MEM /
  HEARTBEAT 三条规则，而 ELC 空转是**「会话活着 + 内存充足 + 单线程磨洋工」**的**健康但病态慢**状态，
  三条**一条都不命中**；它只会在日志里记下 `nflog+300m` 却不告警。且它 heartbeat 时 `exit 12` 等上层复检
  —— **夜里没有上层，等于没监控**。
- **修复（2026-07-16 14:01 已生效并实测）**：新增 `scripts/10_watchdog.sh`，**常驻 tmux 会话 `pan_watch`**：
  - **永不退出**、自我循环（不靠上层复检）；poll=600s（10 min，符合 Phase-2 要求）。
  - **新增规则 A `STALLED`**：`.nextflow.log` 停滞 >45 min → 告警（流程没在推进）。
  - **新增规则 B `LOW_CPU_UTILIZATION`**：load 连续 30 min < 10 → 告警。**这条就是照着 ELC 事故的特征
    写的** —— 会话活着但核在空转，「这不是慢，是设计出了问题」。
  - 保留 SESSION_END / LOW_MEM；**只告警绝不自行 kill**（杀不杀需人工判断，见 CLAUDE.md）。
  - 告警落盘 `logs/watchdog_ALERTS.log`（与巡检流水 `logs/watchdog.log` 分离，便于一眼看异常）。
  - **已实测两条新规则确实触发**（造停滞 3h 的假 nflog + 强制低 load 阈值），且 STALLED 有去重不刷屏
    —— **"看门狗启动了" ≠ "看门狗会告警"，这正是本次事故的同型错误，故必须实测。**
- **举一反三**：`tmux ls` 里每个跑长任务的会话，**都应有一个配对的常驻监控会话**。
  本项目现为 `pan_wgs`(sarek) + `pan_watch`(看门狗) + `pan_down`(下游编排器)。
...
- **确认 `chrM` 上的 variant 也拿到了 ClinVar 注释**（非空）—— 验证 `MT`→`chrM` 改名正确，
  防 §4 记录的静默零注释陷阱复发。
