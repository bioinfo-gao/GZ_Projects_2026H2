...
- **`--genome GATK.GRCh38`（而非本地 GENCODE fasta）—— 为什么：**
  ...
  - **代价（可接受）**：`GATK.GRCh38` 首次运行要下/建 bwa-mem2 index（本地 GENCODE 那份已建好的 index 用不上），
    属一次性成本；换来的是与全部 clinical DB 坐标严格自洽。**iGenomes bundle（fasta+index+known-sites+intervals）
    由 sarek 首次运行自动获取并缓存**。proj4 已验证此路径。
...
| **MarkDuplicatesSpark**（见优化） | **~1–1.5 h** | 中高（多线程） | 优化前单线程 ~3.5 h/样本 |
...
### 提速优化记录（2026-07-15 23:50 应用）
- **`--use_gatk_spark markduplicates`**：把近单线程的 GATK MarkDuplicates（优化前实测量级 ~3.5 h/样本，
  是 CPU 利用率最低的瓶颈段）换成多线程 **MarkDuplicatesSpark**，预计缩到 ~1–1.5 h/样本。依据：wgs skill
  中 13_Ellen 首跑经验（MarkDuplicates 单线程慢，Spark 可提速）。
- 配套 config：`local_resources.config` 增 `withName:'.*MARKDUPLICATES.*' { cpus=16; memory=50.GB }`
  （markdup 期无 bwa index 占用，2×50=100 GB / 125 GB 安全；Spark shuffle spill 到 `/home` 696 G）。
- **应用方式**：nextflow 参数在启动时固定，故编辑 `2_run_sarek.sh` 后 `tmux kill-session` 停旧跑，
  再以 `-resume` 重启 —— 已完成的 prep task（FASTP 等）从 cache 复用，仅重跑未完成的 VEP 下载。
- 脚本 `2_run_sarek.sh` 同时改为**默认 `-resume`**（对首启无害，便于此类 config 变更后无损重启）。
