...
- 2026-07-16 — ⚠ **回退 `--use_gatk_spark markduplicates`**：它触发的单线程 `EstimateLibraryComplexity`
  跑 6.3 h 未完且阻塞流程（反成拖累），改回常规 GATK4_MarkDuplicates；alignment 经 `-resume` 复用。详见 §7。
...
- **修正**：去掉 `--use_gatk_spark`，回到 proj4 验证过的**常规 `GATK4_MARKDUPLICATES`**（metrics 内联、
  无 ELC、~3.5 h/样本但可靠有界）。`-resume` 复用 alignment（31 task 命中 cache），仅 markdup 重做。
  回退后机器满载（load ~56，之前 2.7），HaplotypeCaller 随即开始。
- config：`withName:'.*MARKDUPLICATES.*'` 保留 `memory=50.GB`（减少 temp spill），`cpus=6`（单线程，不需 16）。
- 脚本 `2_run_sarek.sh` 改为**默认 `-resume`**（对首启无害，便于此类 config 变更后无损重启）。
- **结论/给 wgs skill**：`--use_gatk_spark markduplicates` 在**高深度 WGS**上是**陷阱**——省下的 markdup 时间
  被强制追加的 `EstimateLibraryComplexity` 反噬。除非确有 GPU/Spark 集群且已小样验证，**germline WGS 默认用
  常规 MarkDuplicates**。
